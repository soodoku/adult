...
Comparing models:
1) include non-political sites (filtering out mail, facebook, porn, points/benefits sites) + demos
2) filter using only sites predicted to be political/news-related (predicted to be political using ML based on Shallalist & comScore)
3) both of above using proportions (% total visits per respondent) rather than counts
4) demos only for comparison
5) web visits only (counts)
...
# If filtering by politics: Get news_domains -- predicted political/news domains using both shalla & comScore data
...
## RUN BELOW ONLY FOR PROPORTIONS Generate data for each ID --> these are the features for prediction (using proportions)
...
visits_agg <- select(visits_agg, -variable)
head(visit_melted); tail(visit_melted)
head(visit_wide); tail(visit_wide)

## RUN BELOW ONLY FOR COUNTS aggregate by pmxid

x <- as.data.frame(x)
x$pmxid <- visit_domain_id$pmxid
visits_agg <- sapply(unique(visit_domain_id$pmxid), function(i) colSums(x[which(visit_domain_id$pmxid == i), 1:length(domains)])) # 165 5622 5718
...
# demos <- indiv # for demos only
demos <- visits_agg[, grep("pid3", names(visits_agg)):grep("ideo5", names(visits_agg))] # 168:174 167:173 5625:5630 5721:5725
table(demos$educ)
...
demo_names <- colnames(demo_x)

# combine domains + demos
x <- cbind(visits_agg[, 2:(grep("pid3", names(visits_agg))-1)], demo_x) # 3:167 2:166 2:5623
x <- na.omit(x)
demo_x <- na.omit(demo_x)

# or for visits only:
visits_agg <- merge(visits_agg, indiv[, c("pmxid", "pid3")], by = "pmxid", all.x = TRUE)
x <- visits_agg[, 2:5623]
x <- na.omit(x)

setwd("data/yg/predict_pid")
save(x, file = "web_classify_nofilter.RData")
save(x, file = "web_classify_filter.RData")
save(x, file = "web_classify_nofilter_props.RData")
save(x, file = "web_classify_filter_props.RData")
save(x, file = "web_classify_nofilter_justvisits_counts.RData")

### Run 02 predict_pid_run_model.R
